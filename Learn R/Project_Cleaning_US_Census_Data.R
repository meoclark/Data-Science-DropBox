...
us_census <- us_census %>% mutate(White=gsub('\\%','',White))
us_census <- us_census %>% mutate(Black=gsub('\\%','',Black))
us_census <- us_census %>% mutate(Native=gsub('\\%','',Native))
us_census <- us_census %>% mutate(Asian=gsub('\\%','',Asian))
us_census <- us_census %>% mutate(Pacific=gsub('\\%','',Pacific))
...
```{r error=TRUE}
# Remove these extra characters from the columns M and F
